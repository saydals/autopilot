...
| 6   | **비행기 GUI 상시 표시**        | 믹서 종류 무관, Configurator 메인화면에 3D 비행기 모델 항상 표시. 모터 항목 수정시 주의.            |
...
설정 피치각으로 3초간 설정된 피치각 유지 후 목표 고도에 도달하면 귀환을 시작합니다.

설정 피치각이 100이하이면 하강합니다. 즉 이 단계는 이름과는 다르게 작동하며 준비단계입니다.

실제 고도확보는 Fly_home 단계에서 정확하게 설정 고도로 상승 또는 하강합니다.

#### 셔틀 모드 (특화 기능)

단순히 귀환하는 것을 넘어, 특정 지점(A↔B)을 왕복하는 **SHUTTLE** 모드를 지원합니다.  
`ShuttleCount` 값에 따라 왕복 횟수가 결정되며, 무한 반복(SHUTTLE_INFINITE) 설정이 가능하여 간이 무한 자동비행 같은 역할을 합니다.
...
- GPS 신호가 비행중에 잡혀서 Home은 없지만 위치정보가 있는 겨우 무한셔틀 모드는 작동합니다. Home Fix가 없으므로 A, B포인트 생성 방법만 다릅니다.
...
- **아밍 시 자동 전환**: `tryArm()`에서 활성 프로파일이 Profile 3이면 자동으로 Profile 1로 강제 전환된다. 따라서 Profile 3으로 비행할 수는 없지만, Rescue 파라미터 저장 용도로 Profile 3를 사용한다.
- **비행 프로파일**: Profile 1 또는 2를 비행용으로 사용하세요. Rescue 활성 시 내부적으로 Profile 3의 PID 값을 읽어 제어에 사용한다.
- **Profile 1 저장 주의**: Profile 1로 비행하려면 Configurator에서 Profile 1 선택 후 내용을 하나라도 수정하여 저장해야 활성 프로파일로 저장된다.
- **피치 서보 떨림 방지**: Profile 3가 활성 프로파일로 설정된 상태에서 아밍하면 자동으로 Profile 1으로 전환되므로 피치 서보가 떨리는 문제가 발생하지 않는다.
...
## 9. PID Profile AUX Switching

AUX 스위치로 비행 중 PID 프로파일(1/2/3)을 즉시 전환할 수 있습니다. OSD 메뉴나 Configurator 연결 없이 상황에 맞는 PID 셋업으로 즉시 변경 가능합니다.

**⚠ Configurator Adjustments 탭에서는 이 기능이 보이지 않음 .** 

다만 우연의 일치로 선택항목의 제일 마지막 LED 밝기 조절을 선택해 적용하면 PID profile 기능이

실행됩니다. 컨피규레이터 구조와 코드 내용은 모르기 때문에 정확한 원인은 모르나 현재까지는 LED 밝기 조절을 선택해 PID 프로파일 선택이 가능합니다.

cli 설정을 쉽게 하려면 예를 들면 조정 맨 위칸을 아무 항목이나 선택해서 Aux와 범위를 할당한 다음에 CLI에서 adjrainge  줄에서 끝에서 4번째 숫자를 31로 바꾸면 작동합니다. 
